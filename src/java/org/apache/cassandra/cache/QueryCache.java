...
public class QueryCache implements ICache<QueryCacheKey, ColumnFamily>
{
    ...
    public void put(QueryCacheKey qf, ColumnFamily value)
    {
        // TODO do something here... but for now ignore it.
        putIfAbsent(qf, value);
    }

    @Override
    public ColumnFamily get(QueryCacheKey qf)
    {
    ...
    @Override
    public boolean putIfAbsent(QueryCacheKey qf, ColumnFamily value)
    {
        RowCacheKey rk = qf.getRowCacheKey();
        if (cache.putIfAbsent(rk, new QueryCacheValue(Sets.newHashSet(qf.filter), value)))
            return true;
        // We are here since someone added the same or a different filter into cache, but verify
        QueryCacheValue temp2;
        if ((temp2 = cache.get(rk)) != null && temp2.filters.contains(qf.filter))
            return cache.replace(rk, temp2, temp2.cloneWith(qf.filter, value));
        return false;
    }

    @Override
    public boolean replace(QueryCacheKey qf, ColumnFamily old, ColumnFamily value)
    {
