...
import org.apache.cassandra.triggers.TriggerOptions;
...
public final class CFMetaData
{
    ...
    private volatile List<Map<String, String>> triggers = new ArrayList<>();
    ...
    public CFMetaData triggers(List<Map<String, String>> prop) {triggers = prop; cachedTriggers = TriggerOptions.extractClasses(triggers); return this;}
    ...
    public void apply(CFMetaData cfm) throws ConfigurationException
    {
        ...
        triggers(new ArrayList<>(cfm.triggers)); // update trigger as an unit.
    ...
    public RowMutation toSchemaUpdate(CFMetaData newState, long modificationTimestamp, boolean fromThrift)
    {
        ...
        for (Map<String, String> entry : triggers)
            if (!newState.triggers.contains(entry))
                TriggerOptions.deleteColumns(rm, cfName, entry, modificationTimestamp);
        for (Map<String, String> entry : newState.triggers)
            if (!triggers.contains(entry))
                TriggerOptions.addColumns(rm, cfName, entry, modificationTimestamp);
        return rm;
    }
