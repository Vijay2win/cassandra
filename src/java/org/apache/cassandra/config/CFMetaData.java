...
public final class CFMetaData
{
    ...
    public static final CFMetaData SchemaTriggerCf = compile(5, "CREATE TABLE \"" + SystemTable.SCHEMA_TRIGGERS_CF + "\" ("
                                                                + "keyspace_name text,"
                                                                + "column_family text,"
                                                                + "trigger_classes map<text, text>,"
                                                                + "PRIMARY KEY (keyspace_name, column_family)"
    ...
    private volatile Map<String, String> triggers = new HashMap<>();
    ...
    public CFMetaData triggers(Map<String, String> prop) {triggers = prop; cachedTriggers = triggers.isEmpty() ? null : new HashSet<>(triggers.values()); return this;}
    ...
    static CFMetaData copyOpts(CFMetaData newCFMD, CFMetaData oldCFMD)
    {
        ...
        return newCFMD.comment(oldCFMD.comment)
                      ...
                      .triggers(oldCFMD.getTriggers()); // make sure to create new trigger map
    }
    ...
    public Collection<String> getTriggerClass()
    {
    ...
    public void apply(CFMetaData cfm) throws ConfigurationException
    {
        ...
        Map<String, String> triggercopy = new HashMap<>(triggers);
        MapDifference<String, String> tcolumnDiff = Maps.difference(triggercopy, cfm.triggers);
        for (Entry<String, String> entry : tcolumnDiff.entriesOnlyOnLeft().entrySet())
            triggercopy.remove(entry.getKey());
        for (Entry<String, String> entry : tcolumnDiff.entriesOnlyOnRight().entrySet())
            triggercopy.put(entry.getKey(), entry.getValue());
        triggers(triggercopy); // update trigger as an unit.
    ...
    public RowMutation toSchemaUpdate(CFMetaData newState, long modificationTimestamp, boolean fromThrift)
    {
        ...
        MapDifference<String, String> triggDiffrence = Maps.difference(triggers, newState.triggers);
        for (Entry<String, String> entry : triggDiffrence.entriesOnlyOnLeft().entrySet())
        {
            ColumnFamily cf = rm.addOrGet(SystemTable.SCHEMA_TRIGGERS_CF);
            int ldt = (int) (System.currentTimeMillis() / 1000);
            ColumnNameBuilder builder = SchemaTriggerCf.getCfDef().getColumnNameBuilder();
            builder.add(ByteBufferUtil.bytes(cfName)).add(ByteBufferUtil.bytes("trigger_classes")).add(ByteBufferUtil.bytes(entry.getKey()));
            cf.addAtom(new RangeTombstone(builder.build(), builder.buildAsEndOfRange(), modificationTimestamp, ldt));
        }
        for (Entry<String, String> entry : triggDiffrence.entriesOnlyOnRight().entrySet())
        {
            ColumnFamily cf = rm.addOrGet(SystemTable.SCHEMA_TRIGGERS_CF);
            ColumnNameBuilder builder = SchemaTriggerCf.getCfDef().getColumnNameBuilder();
            builder.add(ByteBufferUtil.bytes(cfName)).add(ByteBufferUtil.bytes("trigger_classes")).add(ByteBufferUtil.bytes(entry.getKey()));
            cf.addColumn(builder.build(), ByteBufferUtil.bytes(entry.getValue()), modificationTimestamp);
        }
        return rm;
    }
    ...
    public static CFMetaData fromSchema(UntypedResultSet.Row result)
    {
        ...
        // read triggers.
        cfDef.triggers(SystemTable.getAllTriggers(cfDef.ksName, cfDef.cfName));
