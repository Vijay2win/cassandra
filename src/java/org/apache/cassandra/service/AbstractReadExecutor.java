...
public abstract class AbstractReadExecutor
{
    ...
    AbstractReadExecutor(ReadCommand command, ConsistencyLevel consistency_level) throws UnavailableException
    {
        ...
        this.handler = StorageProxy.getReadCallback(resolver, command, consistency_level, endpoints);
    ...
    public static AbstractReadExecutor getReadExecutor(ReadCommand command, ConsistencyLevel consistency_level) throws UnavailableException
    {
        ...
        {
        case ALWAYS:
            return new SpeculateAlwaysExecutor(command, consistency_level);
        case PERCENTILE:
        case CUSTOM:
    ...
    private static class SpeculativeReadExecutor extends AbstractReadExecutor
    {
        ...
        public SpeculativeReadExecutor(ReadCommand command, ColumnFamilyStore cfs, ConsistencyLevel consistency_level) throws UnavailableException
        {
            super(command, consistency_level);
            this.cfs = cfs;
        }
        ...
        void speculate()
        {
            ...
            if (!handler.await(expectedLatency))
            {
                ...
                if (handler.unfiltered.size() > handler.endpoints.size())
                    endpoint = handler.unfiltered.get(handler.endpoints.size()); // read != RR
                else if (handler.unfiltered.size() > 1)
                    endpoint = handler.unfiltered.get(1); // read == RR, re-send data read to a different node.
                else
                    return;
    ...
    {
        public SpeculateAlwaysExecutor(ReadCommand command, ConsistencyLevel consistency_level) throws UnavailableException
        {
            super(command, consistency_level);
        }
        ...
        void executeAsync()
        {
            int limit = handler.unfiltered.size() >= 2 ? 2 : 1;
            for (int i = 0; i < limit; i++)
            {
                InetAddress endpoint = handler.unfiltered.get(i);
