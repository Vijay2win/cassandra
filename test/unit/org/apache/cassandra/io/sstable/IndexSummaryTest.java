...
public class IndexSummaryTest
{
    ...
    public void testGetKey()
    {
        ...
        for (int i = 0; i < 100; i++)
            Assert.assertEquals(random.left.get(i).key, random.right.getKey(i));
    }
    ...
    private Pair<List<DecoratedKey>, IndexSummary> generateRandomIndex(int size, int interval)
    {
        ...
        for (int i = 0; i < size; i++)
            builder.maybeAddEntry(list.get(i), interval, i);
        IndexSummary summary = builder.build(DatabaseDescriptor.getPartitioner(), interval);
